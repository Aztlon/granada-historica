...
Milestones **M0 — Specification and repository setup** and the local
implementation of **M1 — Working map shell** are complete. The app now opens an
interactive, responsive modern map centered on Granada with the period badge,
layer-control shell, and information drawer. GitHub Pages deployment is
configured and will run from `main` once Pages is set to use GitHub Actions.
...
The deployment workflow builds and publishes `dist/` on pushes to `main`. In
the GitHub repository, select **Settings → Pages → Build and deployment →
GitHub Actions** once. The expected project URL is:
